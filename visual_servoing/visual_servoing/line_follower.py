...
class LineFollower(Node):
   ...
   def relative_cone_callback(self, msg):
       ...
       Ksteering = 1
       drive_cmd.drive.steering_angle = float(np.clip(Ksteering * angle_to_target, -0.34, 0.34))
       ...
       drive_cmd.drive.speed = float(np.clip(speed, 0.0, self.max_speed))

       self.drive_pub.publish(drive_cmd)
       self.get_logger().info(
           f"target=({self.relative_x:.2f}, {self.relative_y:.2f})m "
           f"angle={np.degrees(angle_to_target):.1f}deg "
           f"speed={drive_cmd.drive.speed:.2f} "
           f"steer={np.degrees(drive_cmd.drive.steering_angle):.1f}deg",
